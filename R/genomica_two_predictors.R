#' Genomica - analysis with two predictors
#'
...
#' @param Predictor1 example: c('Treatment') - This is the first predictor (independent variable) according to your study design.
#' @param P1_levels example: c('Control','Treatment1','Treatment2) - If your main predictor is not a numerical variable, this will set the levels for your main predictor (the model will test the comparisons towards the first term in this vector, such as Control Vs Treatment1 and Control Vs Treatment2 in the example).
#' @param Predictor2 example: c('Sex') - This is the second predictor (independent variable) according to your study design.
#' @param P2_levels example: c('Females','Males') - If your second predictor is not a numerical variable, this argument will set the levels for the comparison (the model will test the comparisons towards the first term in this vector, such as Females Vs Males, moreover, the baseline level for the interaction will be in this case Control-Females).
...
#' @param Folder_Name Example: c('Test') - This string will be attached to the folder name in which the results will be generated (e.g.,"Genomica_Output_Two_Predictors_Test/")
#' @import readxl
#' @import writexl
#' @import lmerTest
#' @import dplyr
...
genomica_two_predictors<-function(Data,Metadata,
                                 Predictor1,P1_Levels=c(0),
                                 Predictor2,P2_Levels=c(0),
                                 R_Effects,R1_Levels,
                                 Already_Log10_transformed=c('NO'),
                                 Folder_Name){


  FOLDER<-paste0('Genomica_Output_Two_Predictors_',Folder_Name)
  ...
  ifelse(!dir.exists(FOLDER), dir.create(FOLDER), "The folder 'Genomica_Output_Two_Predictors' already exists")
  ...
  if (length(P1_Levels)==1){
    DF[,Predictor1]<-as.numeric(as.character(DF[,Predictor1]))
  } else if(length(P1_Levels)>1){
    DF[,Predictor1]<-factor(DF[,Predictor1],levels = P1_Levels)
  }
  ...
  for (i in y[1:length(y)]){
    ...
    Results<-matrix(NA,ncol=6,nrow=3,dimnames = list(c(paste0((paste0(i,'_')),c(Predictor1,Predictor2,'Interaction'))),
                                                c("Sum Sq","Mean Sq","NumDF","DenDF","F value","Pr(>F)")))



    if ((sum(DF[,i]))>0){
      model<-lmerTest::lmer(DF[,i]~DF[,Predictor1]*DF[,Predictor2]+(1|Random1),data=DF)
      res<-stats::anova(model)
      Results[1,]<-as.numeric(c(res[1,1:length(res)]))
      Results[2,]<-as.numeric(c(res[2,1:length(res)]))
      Results[3,]<-as.numeric(c(res[3,1:length(res)]))
  ...
  Combined_Results<-dplyr::rename(Combined_Results,'Feature_Predictor'='Feature_Code')
  Combined_Results$Feature_Code<-sub('\\_.*', '', Combined_Results$Feature_Predictor)
  Combined_Results<-Combined_Results[,c(11,1:10)]
  ...
  rows<-c(paste0('(Intercept)',as.character(levels(DF[,Predictor1]))))
  ...
  for (i in x[1:length(x)]){
    ...
    model2<-lmerTest::lmer(DF[,i]~DF[,Predictor1]*DF[,Predictor2]+(1|Random1),data=DF)
    sum<-summary(model2)
  ...
  Combined_Significant$Predictor<-sub(pattern = 'Predictor1',replacement = '',Combined_Significant$Predictor)
  Combined_Significant$Predictor<-sub(pattern = 'Predictor2',replacement = '',Combined_Significant$Predictor)
